...
void test_delaunay_load(void){
    ...
    assert(pid >= 0);
    if( pid == 0 ) {
        FILE* fp_bin = get_fp_bin("./files/csv/100_tree", "./files/bin/.tmp/100_tree.bin", ';');
        list_t* data_list = get_data_csv_bin(fp_bin);
        // Close files
        fclose(fp_bin);
        // Check if data_list is not empty
        tps_assert(list_size(data_list) != 0);
        delaunay_t* delaunay = initiate_delaunay(data_list, "", "./files/delaunay/to_load/delaunay_100.bin");
        // Check if delaunay is not NULL
        tps_assert(delaunay != NULL);
        // Check if delaunay is correct
 ...
 * (Delaunay triangulation bin file has a size of nodes superior to the number of nodes in the csv file)
 */
...
void test_delaunay_save_and_load(void){
    ...
    if( pid == 0 ) {
        ...
        delaunay_t* delaunay_saved = initiate_delaunay(data_list, "./files/bin/.tmp/delaunay_100.bin", "");
        // Check if delaunay is not NULL
        ...
        delaunay_t* delaunay_loaded = initiate_delaunay(data_list, "", "./files/bin/.tmp/delaunay_100.bin");
        // Check if delaunay is not NULL
